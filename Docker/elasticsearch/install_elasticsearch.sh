...
docker run -d --restart unless-stopped --name elasticsearch -p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" docker.elastic.co/elasticsearch/elasticsearch:7.9.2
docker run -d --restart unless-stopped --name kibana --link elasticsearch:elasticsearch -p 5601:5601 docker.elastic.co/kibana/kibana:7.9.2
#  -e elasticsearch.username=selabdev \
#  -e elasticsearch.password=qhdkscjfwj\!@ \

docker run -dit --restart unless-stopped --name logstash -v /docker_data/logstash/pipeline/:/usr/share/logstash/pipeline/ docker.elastic.co/logstash/logstash:7.9.2
...
#docker.elastic.co/beats/filebeat:7.9.2 -e -strict.perms=false \
